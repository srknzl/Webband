# WebBand — sistem notları

CLAUDE.md'nin ayrıntılı hâli. Her bölüm bir mekaniği, tasarım kararını ve o kararın
**ölçülmüş** karşılığını yazar. CLAUDE.md yalnız değişmez kuralları taşır; sebep,
sayı ve tarihçe buradadır.
...
- Yemek tüketimi — düşük kalite (tahıl/ekmek) ve yüksek kalite (et/peynir). Lvl 30+ askerler yüksek kalite alamazsa `debuff` yer (savaşta ×0.7).
  ...
  **Oyuncu da yer (#75)**: `upkeep()` `foodLow = 1` ile başlar — eskiden yalnız parti sayılıyordu,
  ...
  10 asker + oyuncu = günde **6** birim (bkz. aşağıdaki `FOOD_MAN`).
#### Asker günde yarım birim yer (`FOOD_MAN`)
Kişi başı 1 birim fazlaydı: 20 kişilik ordu günde 21 birim (~84 dinar) yiyor, yani
**erzak faturası maaşın iki katı** oluyordu (aynı orduya maaş 40₺). Tek knob
`Game.FOOD_MAN` = **0.5**, `upkeep()`'in hemen üstünde durur; tüketim, "kaç gün yeter"
rozeti, açlık cezası ve künye dökümü zaten hepsi `upkeep()`'ten okuduğu için başka
hiçbir yere dokunulmadı. Oyuncunun kendi karnı **1 birim** kaldı (`foodLow = 1`, #75) —
knob askerin payıdır. Seçkin kademeler oranını korur: lvl 20+ **×1.5** (0.75 birim),
lvl 30+ ayrıca `FOOD_MAN` kadar et/peynir ister, lvl 51 bedava.

Ölçüldü (`upkeep()` doğrudan):

| Grup | Günlük birim | Et/peynir | Maaş | Tahıl bedeli |
|---|---|---|---|---|
| tek kişi | 1 | 0 | 0 | 4₺ |
| 10 × lvl10 | **6** *(eski 11)* | 0 | 20₺ | 24₺ |
| 20 × lvl10 | **11** *(eski 21)* | 0 | 40₺ | 44₺ |
| 20 × lvl20 | 16 | 0 | 200₺ | 64₺ |
| 10 × lvl30 | 9 | 5 | 150₺ | 36₺ |

60 tahıl 10 kişilik gruba **5 gün değil 8 gün** yetiyor. `tools/test.js`'in iki
`foodStock` iddiası bu sayıları kodluyor (need 11→6, days 5→8, need 4→3, needHigh 2→1) —
denge değişince kırmızıya döndüler ve elle güncellendiler, yani regresyon kapısı işliyor.
...
| 🖱️ Kenardan kaydırma (Cihaza göre/Açık/Kapalı) | `Game.edgePan()`; `'auto'` = `!isTouch()` — bkz. "Kenar kaydırması farenin" |
...
**2. Savaş kumandası** (`#touch-ui`, `Game.initTouchUI`). Sol altta **hareket çubuğu**,
sağ altta **nişan çubuğu**, ikisinin arasında 🛡️ blok düğmesi, üstlerinde tek sıra emir
düğmeleri. Ölçüler `:root`'taki üç değişkendedir (`--tui-stick` / `--tui-btn` /
`--tui-lift`) — #86'da küçültüldü. Hiçbiri savaş motoruna yeni bir giriş yolu açmaz:

- Sol çubuk `Input.keys` içindeki `w/a/s/d`'yi yazar (eşik ±0.38) — motor hâlâ tuş okur.
- **Sağ çubuk kılıca yön verir (#88)**: sürüklerken `Input.aim`, parmak kalkınca
  `Battle.playerAttack()`. Sürüklemeden dokunmak son nişan yönüne vurur, yani eski ⚔️
  düğmesinin işi de duruyor. Yay kuşanılmışsa aynı çubuk ok atar (`playerAttack` zaten
  silaha bakar).
- Nişan `Input.aimSync(u)` ile `Input.aim || Input.stick` yönünden türetilip `Input.mouse`'a
  yazılır — savaş motoru hâlâ yalnız `Input.mouse`'a bakar, iki ayrı nişan yolu tutulmaz.
  Sağ çubuğa hiç dokunulmazsa nişan eskisi gibi **hareket** yönüdür (#65 davranışı birebir).
  Fare kıpırdarsa ikisi de sıfırlanır, nişanı yine fare alır.
- Parmakla oynarken imleç yok, o yüzden `Battle.render` oyuncunun önüne **nişan yayı** çizer
  (tek kontur, `swingHalfAngle()` kadar açı) — kılıcın nereye gideceği ancak çizilirse görünür.
- Çubuk makinesi tek yerdedir: `initTouchUI` içindeki `mount(el, knob, onDir, onEnd)`
  kapanışı iki çubuğu da kurar.
...
- İpucu metni cihazı tanır (`Game.isTouch()`): savaş kütüğünde "WASD hareket · Sol tık
  saldırı" yerine "Sol çubuk hareket · Sağ çubuk kılıç · 🛡️ blok", künyede "[Sağ tık/Shift]"
  yerine "🛡 düğmesi".
...
#### Kenar kaydırması farenin, zoom sayfanın değil
Dokunmatikte iki ayrı şey aynı sebepten bozuluyordu: **imleç diye bir şey yok.**

**1. Kenardan kaydırma (edge panning) yalnız farede.** Fareyi ekranın kenarına dayayınca
haritanın kayması doğru davranıştır; ama parmakla oynarken `Input.mouse` son **dokunuşun**
koordinatında kalıyor ve o nokta kenar bandına (40 px) denk geldiyse harita durmadan
kendi kendine kayıyordu — oyuncunun tarifiyle *"oynanamaz hâlde"*. `update`'teki kenar
bloğunun başına tek kapı kondu — `Game.edgePan()`; parmakla kaydırma zaten tek parmak
sürüklemesiyle (#65) yapılıyor, yani kaybedilen bir yetenek yok.

Kapı **ayarlanabilir**: `Game.OPTS.edgePan` = `'auto' | true | false`, ⚙️ Ayarlar'da
**🖱️ Kenardan kaydırma** satırı (Cihaza göre / Açık / Kapalı), `lite` ile aynı üçlü desen.
`'auto'` = `!isTouch()`. Cihaz tahmini iyi bir varsayılandır ama karar değildir: kenar
kaydırması masaüstünde de herkesin sevdiği bir şey değil, ve tablette (coarse pointer +
takılı fare) isteyen olabilir. Diğer iki cihaz knob'u (`isTouch()` giriş yolu için,
`max-width` medya sorguları yerleşim için) dokunulmadı — bkz. aşağıdaki not.

Ölçüldü (imleç sol kenardan 5 px içeride, 10 kare `update(0.016)`, `camera.offsetX`):

| | Cihaza göre | Açık | Kapalı |
|---|---|---|---|
| Fare | **−120** | −120 | **0** |
| Dokunmatik | **0** | −120 | 0 |

**2. Sayfanın tamamını çimdikle zoomlamak kapatıldı.** İki parmakla bütün arayüzü
büyütmek paneli bozuyor (ölçüler ekrana göre hesaplanıyor) ve geri döndürmenin yolu
kalmıyordu. İki kapı gerekti, çünkü tek başına hiçbiri yetmiyor:

| Tarayıcı | Kapı |
|---|---|
| Chrome / Android | `style.css`: `html, body { touch-action: pan-x pan-y; }` — çimdiği **ve** çift dokunuş zoom'unu keser |
| iOS Safari | `Game.init`: `gesturestart` / `gesturechange` / `gestureend` → `preventDefault` (`{ passive: false }`) — Safari `user-scalable=no`'yu yok sayar |

**3. "Masaüstü modu / mobil modu" diye tek bir anahtar yok — ve olmamalı.** Üç ayrı soru
üç ayrı knob'a bağlı, çünkü cevapları aynı cihazda bile ayrışır (tablet: coarse pointer +
geniş ekran + iyi GPU):

| Soru | Knob | Nerede |
|---|---|---|
| Nasıl giriliyor? | `Game.isTouch()` = `pointer: coarse` | `app.js` (`body.touch`, yardım metni, `#touch-ui`) |
| Nasıl yerleşiyor? | `@media (max-width: 820px / 430px)` | `style.css` |
| Ne kadar çiziliyor? | `Game.lite()` (`'auto'` = `isTouch()`) | ⚙️ Ayarlar → 📱 Hafif mod |
| Kenardan kayıyor mu? | `Game.edgePan()` (`'auto'` = `!isTouch()`) | ⚙️ Ayarlar → 🖱️ Kenardan kaydırma |

Üçünü tek moda bağlamak yanlış cevap verir; ayrı bir mobil kod tabanı her değişikliği ikiye
katlar. Eksik olan şey **override**'dı ve o da knob başına çözülüyor: grafik (`lite`) ve
kenar kaydırması (`edgePan`) artık oyuncunun elinde. Yerleşimi zorlayan bir anahtar hâlâ yok;
gerekirse maliyeti `style.css`'teki medya sorgularının seçici listesine bir sınıf eklemektir
(kural gövdeleri kopyalanmaz).

Haritanın **kendi** iki parmak zoom'u duruyor: üç tuval zaten `touch-action: none`
taşıyor (yukarıdaki madde), yani tuvalin üstündeki çimdiği oyun alır, sayfanınkini
tarayıcı almaz.

#### Çete oyuncunun kucağında doğmaz (`SPAWN_SAFE`)
Oyuncu 4500,4500'de başlıyor, `createNPC` ise yarıçapı `Math.random() * 3800` ile, yani
**0'dan** seçiyordu: ilk karede dibinde bir çapulcu çetesi belirebiliyor ve daha harita
görülmeden yakalanıp esir düşülüyordu. `Game.SPAWN_SAFE` = **1500**; `createNPC` en fazla
40 deneme ile bu yarıçapın dışında bir nokta arar (bulamazsa son adayı kabul eder —
kilitlenme yerine kusurlu bir doğum). **Tek kapı burası**: `spawnNPCs`, `spawnBand`,
`spawnTrader` ve günlük yenilenme hepsi `createNPC`'ten geçer, yani dört ayrı yama gerekmedi.

Ölçüldü: oyun başında en yakın haydut **1550** birim; 400 yeni çete doğurulduğunda
**en yakını 1503**, ortalama 2647; başlangıçtaki en yakın altı NPC'nin hepsi lord ya da
kervan (1267–1315 birim) ve **hiçbiri düşman değil**. `Debug.errors 0`.
...
#### Savaş öğreticisi ve açılış ayarları (#88)

**Savaş öğreticisi aynı makinedir, ayrı listedir.** `startTutorial(force, list, key)` üç
argüman alır; harita öğreticisi varsayılanları (`Game.TUTOR` / `TUTOR_KEY`) kullanır, savaş
öğreticisi `Game.BATTLE_TUTOR` (6 adım) ve `BTUTOR_KEY`'i verir. Çağıran `Battle.start`'ın
sonundaki 500 ms'lik `setTimeout`'tur — arena kurulup ilk kare çizildikten sonra. Otomatik
çözüm (`if(auto) return this.autoResolve()`) döngüden **önce** döndüğü için orada hiç çıkmaz.

| Adım | Hedef | Ne anlatır |
|---|---|---|
| 1 | `#battle-canvas` | Nişan imleçtedir (parmakta: sağ çubuk + sarı yay), sol tık savurur, sağ tık/Shift blok |
| 2 | `#tstick` | Sol çubuk — hareket *(masaüstünde atlanır)* |
| 3 | `#tastick` | Sağ çubuk — kılıcın yönü *(masaüstünde atlanır)* |
| 4 | — (ortada) | Emir fırsatları savaşın içinde doğar |
| 5 | `#btn-surrender` | Teslim ol her an açık; sen ölürsen bayılırsın, savaş bitmez |
| 6 | — (ortada) | Zırha karşı mızrak/ok/topuz, blok yalnız önden korur |

**Cihaz ayrımı bedavaya gelir**: `tutorStep` hedefi bulunmayan ya da görünmeyen
(`!el.offsetParent`) adımı zaten atlıyordu, `#touch-ui` çocukları da masaüstünde
`display: none` — yani 2. ve 3. adım farede kendiliğinden düşer, 6 adım 4 adıma iner.
Cihaz dallanması yazılmadı.

**Öğretici açıkken savaş durur.** `Battle.paused` yalnız `update`'i geçer, `render` dönmeye
devam eder — okunacak şey ekranda dururken adam öldürülmesin, ama arena da kaybolmasın.
`startTutorial` bayrağı `Battle.active` ise kurar, `endTutorial()` kaldırır; adımlar arası
`endTutorial(true)` **kaldırmaz** (kutu değişir, savaş duruyor kalır). `dt` zaten 0.05'e
kırpıldığı ve `last = t` her karede yazıldığı için devam ederken sıçrama olmaz.

**Açılış ayarları** karakter yaratma sihirbazının son adımındadır (`renderDiffStep`): zorluk
ve 📱 Hafif mod. İkisi de `Game.setOpt` kapısından geçer, yani ⚙️ Ayarlar'daki satırlarla tek
kod yolunu paylaşır; seçim adımı kapatmaz (`pickDiff` adımı yeniden çizer), ilerleten şey
**Sonraki** düğmesidir (`diffStepDone`). Dil bu adıma girmez — ilk açılıştaki `#lang-ask`
perdesi zaten soruyor.
...
**Zorluk oyuna girerken sorulur.** Ayarlar menüsünü kimse açmadan önce kararın verilmesi
gerekiyordu; karakter yaratma sihirbazı bu yüzden 6 değil **7 adım** (`renderDiffStep`,
sancaktan sonra, özetten önce). Ayrı bir tablo yok — aynı `Game.DIFFS`'ten okur ve aynı
`Game.setOpt('difficulty', k)` kapısından yazar, yani ⚙️ Ayarlar'daki satırla tek kod
yolunu paylaşır. Özet ekranında `⚔️ Zorluk Zor` satırı olarak görünür.
Ölçüldü: adım başlığı `Adım 7/7`, ORTA seçili geliyor, seçim sonrası
`Game.opt('difficulty')` = `'hard'` ve `Game.diff().name` = `Zor`, `Debug.errors 0`.

**Knob'un oynanıştaki karşılığı ölçüldü** (`tools/harness.js`, gerçek `Battle.update(1/60)`
adım adım işletilir; hücre başına 24 tur, tohum 100+i, `Battle.endBattle` saplandı,
oyuncu 100 can ve **etkin bir bot**: en yakın düşmana yürür, nişan alır, bekleme sayacı
dolunca savurur):

| Senaryo | Ölçüm | Kolay | Orta | Zor |
|---|---|---|---|---|
| 10× lvl10 Milis vs 12 Çapulcu | zafer | **%79** | %13 | **%0** |
| | oyuncunun can kaybı | 32.6 | 92.5 | 100 |
| | asker kaybı | 3.7 | 9.1 | 10 |
| 20× lvl10 Milis vs 20 Çapulcu | zafer | **%79** | %13 | **%0** |
| | asker kaybı | 7.8 | 18.5 | 20 |
| Tek başına oyuncu vs 3 Kurt | dayanma süresi | **13.3 sn** | 8.6 sn | **6.7 sn** |

Yani kademe tek yönlü ve etkisi tabloyla birebir örtüşüyor (Kolay'da hayatta kalma
≈ 1/0.6 ≈ **×1.55**). **Kolay tek başına bir oyuncuyu 3 kurttan kurtarmaz** — yalnız
~%55 daha fazla zaman kazandırır; mutlak zafer oranları sert görünüyor çünkü betikteki
bot ne blok yapıyor ne geri çekiliyor. Gerçek oyuncunun elindeki iki şey (kalkan ve kaçış)
ölçüme girmiyor.
...
|---|---|---|---|---|---|
| Boş gezen ordu | 37 | **−16.1** | 20₺ | **12₺** | 0/10 |
| Ticaret rotası | 8 | **−16.5** | 20₺ | **14₺** | 0/10 |
| Tımar sahibi | 6721 | **+95.3** | 20₺ | **31₺** | 11/10 |

*(`FOOD_MAN` sonrası ölçüm. Erzak kalemi her betikte düştü — 14→12, 18→14, 49→31 — ama
"gün başına" yalnız **tımarda** iyileşti: +79.1 → **+95.3**, yani tasarruf edilen ~18₺
doğrudan kâra yazıldı. Diğer iki betik zaten sıfır dinarla bitiyor; orada ölçülen şey
kârlılık değil "kaç günde battı", o yüzden −14.6 → −16.1 farkı anlamlı değildir.
Okunacak sonuç değişmedi: **10 kişilik ordu geliri olmadan yaşamaz** — ama fatura artık
maaşın iki katı değil, maaşla aynı mertebede.)*
